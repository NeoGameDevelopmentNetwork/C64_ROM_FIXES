...
; ******** Source: garbcol.asm
   ...
   190  0881 e0091500c1b6       !wo part2_real,part2_real_end-part2_real,part2
   191  0887 f5091b0053bf       !wo part3_real,part3_real_end-part3_real,part3
   192  088d 100a4c0060e4       !wo part4_real,part4_real_end-part4_real,part4
   193  0893 5c0a1700bae4       !wo part5_real,part5_real_end-part5_real,part5
   ...
   272  08d1 20a7e4             	jsr setptr		; damit ptr setzen
   273                          
   ...
   285  08e5 20a7e4             	jsr setptr		; ptr = A/X
   286                          
   ...
   343                          
   344  0929 20a7e4             cw1	jsr setptr		; A/X -> ptr (Alt-String-Zeiger)
   345                          
   346  092c a000               	ldy #0
   ...
   559                          part2:
   560                          
   561                          part2_continue = $b6d6
   562  09e0 4cd6b6             	jmp part2_continue
   563                          
   ...
   586  09f2 4c53bf             	jmp part3
   587                          				; weiter an anderer Stelle!
   588                          blapast
   589                          !if blapast > part2_continue {
   590                          	!error "part2 ist zu lang!"
   591                          }
   592                          }
   593                          part2_real_end
   594                          
   ...
   606  09fc 8559               	sta aryptr+1		; Folge-Array H
   607                          
   608  09fe 28                 	plp			; Arraytyp:
   609  09ff 300c               	bmi +		; kein String-Array
   610  0a01 8a                 	txa
   611  0a02 1009               	bpl +		; kein String-Array
   612                          
   613  0a04 c8                 	iny			; Y=4
   614  0a05 b122               	lda (ptr),y		; Anzahl der Dimensionen (< 126 !)
   615  0a07 0a                 	asl 			; *2
   616  0a08 6905               	adc #5			; + 5 (Var.Name+Offset+Dimensionen)
   617  0a0a 4c8ce4             	jmp backlinkarr2
   618                          
   619  0a0d 4ca3e4             +	jmp blaskip_cont 
   620                          }
   621                          part3_real_end
   622                          
   623                          
   624                          
   625                          part4_real
   626                          !pseudopc $e460 { ; +7 bytes wegen part4pre
   627                          
   628                          part4:
   629                          
   630                           	; Einschaltmeldung kürzen; analog zu 2. Einschalttext
   631                          	; " bytes free"\n\0"
   632  0a10 20425954455320     	!byte  $20, $42, $59, $54, $45, $53, $20
   633  0a17 465245450d00       	!byte  $46, $52, $45, $45, $0d, $00
   634                          	; 6 Bytes frei für anderen Patch
   635  0a1d aaaaaaaaaaaa       	!byte  $aa, $aa, $aa, $aa, $aa, $aa
   636                          
   637                          	; "° commodore 64 basic v2 \0"
   638  0a23 9320434f4d4d4f44   	!byte  $93, $20, $43, $4f, $4d, $4d, $4f, $44
   639  0a2b 4f52452036342042   	!byte  $4f, $52, $45, $20, $36, $34, $20, $42
   640  0a33 4153494320563220   	!byte  $41, $53, $49, $43, $20, $56, $32, $20
   641  0a3b 00                 	!byte  $00
   642                          
   643                          backlinkarr2:		; $e48c
   644  0a3c 6522               	adc ptr			; auf 1. Element ...
   645  0a3e 8522               	sta ptr 
   646  0a40 9002               	bcc +
   647  0a42 e623               	inc ptr+1 
   648  0a44 a623               +	ldx ptr+1		; positionieren
   649                          
   650  0a46 e459               blanext	cpx aryptr+1		; Array-Ende erreicht?
   651  0a48 d004               	bne blaset		; nein, Backlink setzen
   652  0a4a c558               	cmp aryptr
   653  0a4c f007               	beq blafinish		; Array fertig, Bugfix 2: Z-Flag löschen!
   654                          blaset
   655  0a4e 20e6b5             	jsr backlink		; Backlink setzen
   656  0a51 d0f3               	bne blanext		; immer (High-Byte != 0)
   657                          
   658                          blaskip_cont
   659  0a53 a558               	lda aryptr		; Zeiger auf Folge-Array
   660                          blafinish
   661  0a55 a659               	ldx aryptr+1 		; Z=0 sicherstellen
   662                          
   663  0a57 8522               setptr	sta ptr			; Arbeitszeiger setzen
   664  0a59 8623               	stx ptr+1
   665  0a5b 60                 	rts			; immer Z=0
   666                          }
   667                          part4_real_end
   668                          
   669                          
   ...
   675                          part5:
   676                          
   677                          ;**** String Allocation (Fortsetzung)
   678                          ;
   679                          ;	in: 	TOS		; Länge
   680                          ;		fretop		; String-Adresse
   681                          ;	out:	fretop		; String-Adresse
   682                          ;		strptr		; String-Adresse (wird nicht verwendet)
   683                          ;				; (bei alternate_stralloc eventuell mit
   684                          ;				; inkrementiertem High-Byte)
   685                          ;		A		; Länge
   686                          ;		X,Y		; String-Adresse (L,H)
   687                          ;	called:	allocate (in Fortsetzung)
   688                          
   689                            !ifndef alternate_stralloc {
   690                          stralloc:
   691                          	sta strptr		; strptr = A/X = FRETOP
   692                          	stx strptr+1
   693                          	tax			; A in X aufheben
   694                          	pla			; Länge temp. vom Stack 
   695                          	pha			; wieder auf Stack, nun auch in A
   696                          	tay			; Index=Länge (Backlink-position)
   697                          	sta (fretop),y		; Backlink L = String-/Lückenlänge
   698                          	iny			; Y=Länge+1
   699                          	bne sa1			; wenn Länge=255, dann
   700                          	inc fretop+1		; Überlauf, aber nur temporär!
   701                          
   702                          sa1	lda #$ff		; Backlink H = Markierung "Lücke"
   703                          	sta (fretop),y
   704                          	ldy strptr+1
   705                          	sty fretop+1		; Überlaufkorr. rückgängig
   706                          	pla			; Länge vom Stack nehmen
   707                          	rts
   708                          
   709                            } else {
   710                          ; alternative, etwas kürzere Variante (-3 T, -2 B)
   711                          
   712                          stralloc:
   713  0a5c 8535               	sta strptr		; strptr = A/X = FRETOP
   714  0a5e 8636               	stx strptr+1
   715  0a60 aa                 	tax			; A in X aufheben
   716  0a61 68                 	pla			; Länge temp. vom Stack 
   717  0a62 48                 	pha			; wieder auf Stack, nun auch in A
   718  0a63 a8                 	tay			; Index=Länge (Backlink-position)
   719  0a64 9135               	sta (strptr),y		; Backlink L = String-/Lückenlänge
   720  0a66 c8                 	iny			; Y=Länge+1
   721  0a67 d002               	bne sa1			; wenn Länge=255, dann
   722  0a69 e636               	inc strptr+1		; Überlauf, aber nur temporär!
   723                          
   724  0a6b a9ff               sa1	lda #$ff		; Backlink H = Markierung "Lücke"
   725  0a6d 9135               	sta (strptr),y
   726  0a6f a434               	ldy fretop+1		; in Y String-Adresse High-Byte
   727  0a71 68                 	pla			; Länge vom Stack nehmen
   728  0a72 60                 	rts
   729                          				; Hier weicht strptr+1 u.U. von fretop+1 ab,
   730                          				; was aber kein Problem darstellt, da
   731                          				; es im BASIC-Interpreter keine Stellt gibt,
   732                          				; die nach einem allocate-Aufruf den
   733                          				; Pointer strptr/strptr+1 verwendet!
   734                            }
   735                          }
   736                          
   737                          part5_real_end
   738                          
   739                          
   740                          ; Einsprungspunkt an korrekter Position?
   741                          
   742                          ; Kann erst nach dem Label docollect gemacht werden!
   743                          
   744                          !if (garcoll != docollect) {
   745                          	!error "Einstiegspunkt nicht an richtiger Stelle! ",garcoll,"!=",docollect
   746                          }
